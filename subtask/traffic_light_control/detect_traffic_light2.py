...
class DetectTrafficLight(Node):
    ...
    def __init__(self):
        ...
        # 추가
        self.pub_color = self.create_publisher(String, '/detect/traffic_light_color', 1)
    ...
    def publish_traffic_light_color(self, detected_colors):
        """
        감지된 색상 리스트를 받아 '/detect/traffic_light_color' 토픽으로 발행
        detected_colors: ['RED', 'YELLOW', 'GREEN'] 형태의 리스트
        색상이 없으면 'NONE' 발행
        """
        msg = String()
        msg.data = ','.join(detected_colors) if detected_colors else 'NONE'
        self.pub_color.publish(msg)


    def find_traffic_light(self):
        detected_colors = []  # 감지된 색상을 저장할 리스트
        ...
        if detect_red:
            ...
            detected_colors.append('RED')  # RED 감지 시 리스트에 추가
        ...
        if detect_yellow:
            ...
            detected_colors.append('YELLOW')  # YELLOW 감지 시 리스트에 추가
        ...
        if detect_green:
            ...
            detected_colors.append('GREEN')  # GREEN 감지 시 리스트에 추가
        ...
        if self.pub_image_type == 'compressed':
            ...
            # 퍼블리시 직후 터미널에 감지된 색상 출력
            print(f"Detected traffic light colors: {', '.join(detected_colors) if detected_colors else 'No light detected'}")
        
        self.publish_traffic_light_color(detected_colors)
