...
class DetectLevelNode(Node):
    ...
    def __init__(self):
        ...
        # 발행 메세지
        self.pub_crossing_state = self.create_publisher(
            String, '/detect/level_crossing_state', 10)
    ...
    def publish_state(self):
        ...
        self.pub_crossing_state.publish(msg)
    ...
    def find_rect_of_level(self, mask):
        ...
        if len(keypts) >= 3:
            for kp in keypts:
                mean_x += kp.pt[0] / len(keypts)
                mean_y += kp.pt[1] / len(keypts)
